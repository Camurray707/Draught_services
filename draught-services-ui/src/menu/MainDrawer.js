...
import {
	...
	containerComponents,
	transactionsContainerComponents,
...
const TransactionsContainerListItems = (props) => {
	return (
		<div>
			{props.menuItemTitles.map((title) => (
				<ListItem button onClick={() => props.onClick(title)} key={title}>
				...
				<List>
					...
					<Collapse in={transactionOpen} timeout="auto" unmountOnExit>
						<List component="div" disablePadding sx={{pl: 4}}>
							<TransactionsContainerListItems
								selectedItem={selectedItem}
								onClick={handleSelectedItem}
								menuItemTitles={transactionsContainerComponents().map(
									(comp) => comp.title
								)}
							/>
						</List>
